...
function []=PDosPlotFun4(savefilename,lorbitnum)
% global Material
...
switch lorbitnum %取值为21 22 3 4 分别代表Al N Er 兼容4轨道
    case 21
     %%%%%%%%%%%能带平移%%%%%%%%%   
    PhysicsVar(:,1)=PhysicsVar(:,1)+Translationup;
    PhysicsVar(:,5)=PhysicsVar(:,5)+Translationup;
    PhysicsVar(:,3)=PhysicsVar(:,3)+Translationdown;
    PhysicsVar(:,7)=PhysicsVar(:,7)+Translationdown;
    %%%%%%%%%%%%%%能带平移%%%%%%%%%%%
...
plot(PhysicsVar(:,1),PhysicsVar(:,2),'k',...
    ...
    case 3
    %%%%%%%%%%%能带平移%%%%%Translationup Translationdown%%%%   
    PhysicsVar(:,1)=PhysicsVar(:,1)+Translationup;
    PhysicsVar(:,5)=PhysicsVar(:,5)+Translationup;
    PhysicsVar(:,13)=PhysicsVar(:,13)+Translationup;
    PhysicsVar(:,3)=PhysicsVar(:,3)+Translationdown;
    PhysicsVar(:,7)=PhysicsVar(:,7)+Translationdown;
    PhysicsVar(:,15)=PhysicsVar(:,15)+Translationdown;
    %%%%%%%%%%%%%%能带平移%%%%%%%%%%%   
...
plot(PhysicsVar(:,1),PhysicsVar(:,2),'k',...
    ...
    %#####指定图例####
     ...
     %#######使图像紧致####################
      ...
      PhysicsVar(:,1)=PhysicsVar(:,1)+Translationup;    
      PhysicsVar(:,5)=PhysicsVar(:,5)+Translationup;
      PhysicsVar(:,9)=PhysicsVar(:,9)+Translationup;
      PhysicsVar(:,13)=PhysicsVar(:,13)+Translationup;
      PhysicsVar(:,3)=PhysicsVar(:,3)+Translationdown;
      PhysicsVar(:,7)=PhysicsVar(:,7)+Translationdown;
      PhysicsVar(:,11)=PhysicsVar(:,11)+Translationdown;
      PhysicsVar(:,15)=PhysicsVar(:,15)+Translationdown;
      %%%%%%%%%%%能带平移%%%%%%%%% 
plot(PhysicsVar(:,1),PhysicsVar(:,2),'k',...
      PhysicsVar(:,5),PhysicsVar(:,6),'b',...
